...
check_wgetcurl(){
	...
	[ -z "$1" ] && (apk del wget wget-nossl 2>/dev/null; apk add wget ; check_wgetcurl 1 ;return)
...
doupx(){
	...
	#tar xvJf
	# 优化 xz 检查命令
	which xz || (apk info -e xz || apk update && apk add xz) || (echo "Failed to install xz, it's required for installing upx." && EXIT 1)
